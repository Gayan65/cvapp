...
export const userUpdate = async (fname, lname, hash, user_id) => {
  const sqlQuery =
    "UPDATE users SET fname = ?, lname = ?, hash = ? WHERE user_id = ?;";
  const [rows] = await mySqlPool.query(sqlQuery, [fname, lname, hash, user_id]);
  return rows;
};
